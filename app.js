...
function stats(){let all=VOCAB.flatMap(v=>MODES.map(m=>({v,m,x:progress(v,m)}))),dueN=all.filter(a=>a.x&&due(a.v,a.m)).length,mastered=all.filter(a=>isMastered(a.x)).length,unseen=all.filter(a=>!a.x).length,pending=all.length-mastered;$('#dueCount').textContent=dueN;$('#masteredCount').textContent=mastered;$('#streakCount').textContent='残り1日';$('#progressBar').style.width=`${mastered/all.length*100}%`;$('#planTitle').textContent='本日完結プラン';$('#planDetail').textContent=`今日 ${new Date(state.plan.target).toLocaleDateString('ja-JP')}中・要学習 ${pending}技能（未学習 ${unseen}）・期限到来 ${dueN}・定着 ${mastered}/${all.length}`}
...
function start(){
  ...
  let reviewItems=items.filter(x=>progress(x.card,x.mode)&&!isMastered(progress(x.card,x.mode))),newItems=items.filter(x=>!progress(x.card,x.mode));
